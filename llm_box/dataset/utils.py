...
def getting_prompt(indices, example_data):
    prompt = "\n\n".join([example_data[i]["source"] + example_data[i]["target"] for i in indices])
    return prompt
...
def generate_instructions(queries, n):
    gpt_config = {
...
def eval_construct(full_demo, eval_data, prompt, prompt_eval_template):
    eval = prompt_eval_template.replace('[DEMO]', full_demo).replace('[PROMPT]',
                                                                     prompt).replace('[INPUT]', eval_data["source"])
