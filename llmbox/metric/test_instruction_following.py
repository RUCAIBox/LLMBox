...
class Test_instruction_following(Metric):
    ...
    def __call__(self, predictions, references):
        if self.type == "strict":
            all_following_list = Test_instruction_following._get_strict_score_list(predictions, references)
        elif self.type == "loose":
            all_following_list = Test_instruction_following._get_loose_score_list(predictions, references)
        else:
            raise ValueError(f"{self.type} does not exists, please check the type")
        self._last_score_lists = {'Accuracy': all_following_list}
        score_list = []
    ...
    @staticmethod
    def _get_strict_score_list(predictions, references):
        all_following_list = []
        for pred, refer in zip(predictions, references):
            instruction_list = refer["instruction_id_list"]
            instance_following_list = []
            for index, instruction_id in enumerate(instruction_list):
                instruction_cls = INSTRUCTION_DICT[instruction_id]
                instruction = instruction_cls(instruction_id)
                instruction.build_description(**refer["kwargs"][index])
                args = instruction.get_instruction_args()
                if args and "prompt" in args:
                    instruction.build_description(prompt=refer["prompt"])

                if pred.strip() and instruction.check_following(pred):
                    instance_following_list.append(True)
                else:
                    instance_following_list.append(False)
            all_following_list.append(instance_following_list)
        return all_following_list

    @staticmethod
    def _get_loose_score_list(predictions, references):
        all_following_list = []
        for pred, refer in zip(predictions, references):
            r = pred.split("\n")
            ...
            for index, instruction_id in enumerate(instruction_list):
                ...
                for r in all_responses:
                    if r.strip() and instruction.check_following(r):
                        is_following = True
                        break
                instance_following_list.append(is_following)
            all_following_list.append(instance_following_list)
        return all_following_list
...
_STARTER_OPTIONS = (
    ...
    "As I see it", "According to me", "As far as I'm concerned", "To my understanding", "In my view",
...
INSTRUCTION_CONFLICTS = {
    ...
    set(INSTRUCTION_DICT.keys()).difference({
        _KEYWORD + "forbidden_words", _KEYWORD + "existence", _LANGUAGE + "response_language", _FORMAT + "title",
        _PUNCTUATION + "no_comma"
    }),
    _COMBINATION + "repeat_prompt":
